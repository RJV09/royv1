...
const ctx = async (channel, content, color = "#FF66CC", timeout = 5000) => {
...
module.exports = {
  name: "flat",
  aliases: ["flat","default","filter-reset","reset"],
  category: "Filters",
  desc: "Removes all filters from the music.",dev: false,
  options: {
    owner: false,
    inVc: true,
    sameVc: true,
  ...
  run: async ({ client, message }) => {
    try {
      ...
      await applyEqualizerPreset(player, "default");
      return ctx(
        message.channel,
        "Removed all filters from the music.",
        client.config.color,
        10000
      );
    } catch (e) {
