...
async function createQualityImage(currentQuality, theme = "neon") {
  ...
  ctx.fillText("SOFIA DOLBY SETTING", 600, 100);
  ctx.font = "300 30px 'Arial', sans-serif";
  ctx.fillStyle = colors.accent;
  ctx.fillText("EXPERIENCE", 600, 140);

  // Quality circles and labels
  const qualities = ["Low", "Medium", "High"];
  const y = 350; // Y-axis position for circles

  qualities.forEach((quality, index) => {
    const x = 300 + index * 300; // X-axis position for circles
    const radius = 100;

    // Create dynamic color for circles
    ...
    // Fill inner circle with gradient
    ...
    // Add text for the quality
    ctx.font = "bold 30px 'Arial', sans-serif";
    ctx.fillStyle = colors.text;
    ctx.fillText(quality, x, y + 10);

    // Highlight the current quality
  ...
  return new AttachmentBuilder(buffer, {
    name: "sofia_tejas_quality_settings.png",
  });
}
      ...
      const updateMessage = async (newQuality) => {
        try {
          ...
          const embed = new EmbedBuilder()
            .setTitle("🎧 Sofia HD Audio Experience")
            .setDescription(
            ...
            .setImage("attachment://sofia_tejas_quality_settings.png");
