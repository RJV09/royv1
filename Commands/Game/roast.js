...
module.exports = {
    ...
    category: "Games",
    permission: "",
    desc: "Roast someone or the air with a random roast",
    dev: false,
    options: {
        owner: false,
        inVc: false,
    ...
    run: async ({ client, message, args }) => {
        try {
            ...
            // If no one is mentioned, roast the air and delete the message after 2 seconds
            if (!mentionedUser) {
                const airRoastMsg = await message.reply("Damn, You are trying to roast the air..??");

                // Wait 2 seconds and delete the message
                setTimeout(() => {
                    airRoastMsg.delete();
                }, 2000);

                // Send a random roast from the list
                const randomRoast = roasts[Math.floor(Math.random() * roasts.length)];
                await message.reply(randomRoast);
            } else {
                // Roast the mentioned user directly
                const randomRoast = roasts[Math.floor(Math.random() * roasts.length)];
                await message.reply(`<@${mentionedUser.id}>, ${randomRoast}`);
            }
        } catch (e) {
            console.error(e);
