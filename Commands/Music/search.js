  ...
  run: async ({ client, message, args }) => {
    try {
      ...
      if (!urlRegex.test(query)) {
        let msgssa = await message.channel.send({
          content: `- Searching Songs on Sofia Please Wait!`,
        });
        ...
        const msg = await sendInitialMessage(message, songs, 0).then(() =>
          msgssa.delete()
        );
...
async function searchSongs1(client, query, limit, message) {
  try {
...
async function searchSongs(client, query, message) {
  try {
    const res1 = await client.kazagumo.search(query, {
      source: "ytsearch:",
      requester: message.author,
    });
    let firstTrack = res1.tracks[0];
    //console.log(`First Track: ${firstTrack.title}`);
    if (!res1.tracks || res1.tracks.length === 0) return [];
    let recURL = `https://www.youtube.com/watch?v=${res1.tracks[0].identifier}&list=RD${res1.tracks[0].identifier}`;
    const res2 = await client.kazagumo.search(recURL, {
      source: "ytsearch:",
      requester: message.author,
    });
    let nineTracks = res2.tracks.slice(1, 10);
    // console.log(`Nine Tracks: ${nineTracks.map((track) => track.title).join("\n")}`);
    return [firstTrack, ...nineTracks];
  } catch (error) {
    console.error("Error in searchSongs:", error);
    return [];
  }
...
function drawTitle(ctx, canvas) {
  ...
  ctx.fillText("Sofia Music System", canvas.width / 2, 50);
  ctx.font = "18px Arial";
  ctx.fillStyle = "#cccccc";
  ctx.fillText("Sofia Music - The Perfect Music Partner", canvas.width / 2, 80);
  ctx.font = "24px Arial";
  ctx.textAlign = "left";
}
...
function createAddedToQueueEmbed(song, message, client) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `[${song.title ?? "Unknow Sofia5 track"}](${
...
//   aliases: ["p"],
//   category: "Music",
//   desc: "Play a song of your favorite choice",dev: false,
//   options: {
//     owner: false,
//     inVc: true,
//     sameVc: false,
...
//       iconURL: user.displayAvatarURL({ dynamic: true }),
//     })
//     .setDescription(`[${song.title?? "Unknow sofia track"}](${song.uri?? "https://www.google.com"}) ( <@${user.id}> )`)
//     .setColor("#FF66CC")
//     .setThumbnail(song.thumbnail)
//     .addFields(
//       { name: "Duration", value: formatDuration(song.length), inline: true },
...
//       .setEmoji("1009435997534175323"),
//     new ButtonBuilder()
//       .setURL(`https://discord.com/oauth2/authorize?client_id=904317141866647592&permissions=8&scope=bot%20applications.commands`)
//       .setStyle(5)
//       .setLabel("Invite Me")
//       .setEmoji("1077942469108445297")
//   );
