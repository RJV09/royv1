...
const { ryzenRecommendations } = require("../../Handlers/ryzenrec");
...
async function searchSongs(client, query, limit, message) {
  try {
    ...
    const ryzenRex = await ryzenRecommendations(seedTrack, limit - 2);
    const allTracks = await Promise.all(
      ryzenRex.map(async (rec) => {
