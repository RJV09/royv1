...
async function handleSlashCommand(client, interaction) {
  const command = client.slashLoader?.getSlashCommand(interaction.commandName);
  ...
  try {
    ...
    if (interaction.commandName === 'help' && options.length > 0) {
      const commandOption = options.find(opt => opt.name === 'command');
      if (commandOption) {
        args.length = 0; // Clear args
        args.push(commandOption.value);
      }
    }
    ...
    // Parse user mentions from options
    for (const option of options) {
      if (option.type === 6) { // USER type
        const user = await client.users.fetch(option.value).catch(() => null);
        const member = interaction.guild?.members.cache.get(option.value);

        if (user) {
          fakeMessage.mentions.users.set(option.value, user);
        }
        if (member) {
          fakeMessage.mentions.members.set(option.value, member);
        }
      } else if (option.type === 7) { // CHANNEL type
        const channel = interaction.guild?.channels.cache.get(option.value);
        if (channel) {
          fakeMessage.mentions.channels.set(option.value, channel);
        }
      }
    }

    // Add first() method to mentions
    fakeMessage.mentions.users.first = () => {
      return Array.from(fakeMessage.mentions.users.values())[0] || null;
    };
    fakeMessage.mentions.members.first = () => {
      return Array.from(fakeMessage.mentions.members.values())[0] || null;
    };
    fakeMessage.mentions.channels.first = () => {
      return Array.from(fakeMessage.mentions.channels.values())[0] || null;
    };

    // Check if interaction needs to be deferred for long-running commands
    const longRunningCommands = ['play', 'search', 'sprec', 'sp', 'lyrics'];
    if (longRunningCommands.includes(command.name)) {
      await interaction.deferReply();
    }

    // Get guild data for slash commands
    let guildData = null;
    if (interaction.guild) {
      const Guild = require("../../Models/Guild");
      guildData = await Guild.findOne({ guildId: interaction.guild.id }) || { prefix: client.config.def_prefix };
    }

    // Execute the command
    await command.run({
      client,
      message: fakeMessage, // Pass interaction as message for compatibility  
      args,
      interaction,
      guildData
    });
...
async function Like(interaction, client) {
  ...
  if (favstracks.includes(player.queue.current.uri)) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `<:dislike:1314666959337558049> You Have **Disliked** [**${player.queue.current.title}**](${player.queue.current.uri})`
      );
    return interaction.reply({ embeds: [embed], ephemeral: true });
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `<a:heart:1314664788403687467> You **Liked** [**${player.queue.current.title}**](${player.queue.current.uri})`
    );
  return interaction.reply({ embeds: [embed], ephemeral: true });
}
      ...
      try {
        ...
        try {
          ...
          const em = new EmbedBuilder()
            .setColor(client.config.color)
            .setTitle(`<a:playing:1314189382684770354> Sofia Music Invite`)
            .setURL(`https://discord.gg/hindustani`)
            .setThumbnail(player.queue.current.thumbnail)
            .setDescription(
              `<a:music:1265005782509031586> **${interaction.user.globalName}** has invited you to join **${memberVC.name}** Music Party in **${interaction.guild.name}**\n\nPlaying :  [${player.queue.current.title}](${channellink}) by ${player.queue.current.author}`
            )
            .addFields(
