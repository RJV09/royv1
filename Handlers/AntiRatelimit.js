...
class AntiRatelimit extends EventEmitter {
  ...
  async _notifyBan(message, timeLeft) {
    ...
    await message.author
      .send({
        content: `You are temporarily banned from Sofia for ${timeLeft} seconds.`,
      })
      .catch(async () => {
  ...
  _trackUserViolation(userId, message) {
    ...
    if (newViolations >= this.options.banLimit) {
      ...
      this._notifyUser(
        message,
        "> You are sending messages too quickly. Please slow down or you will be temporarily banned from **Sofia**."
      );
  ...
  _banUser(userId, message) {
    ...
    this._notifyUser(
      message,
      "- You have been **Temporarily Banned** from Sofia for 10 minutes. For Trying to bypass Sofia Anti ratelimit."
    );
    this.bannedMessageSent.add(userId);
    setTimeout(() => {
      this.bannedUsers.delete(userId);
      ...
      user
        .send({
          content:
            "## You have been **Unbanned** from Sofia Service. Please do not bypass the anti ratelimit again.",
        })
        .catch(() => {
          message.channel
            .send({
              content: `<@${userId}> You have been **Unbanned** from Sofia Service. Please do not bypass the anti ratelimit again.`,
            })
            .then((sent) => {
              setTimeout(() => sent.delete(), 5000);
            });
