...
const sofiaDispatcher = require("./bot");

module.exports = class Loader {
  constructor(client) {
    this.client = client;
    this.eventsDir = path.resolve("./Events/Client");
    this.Load();
  }
  async Load() {
    await Promise.all([
  ...
  async handleDispatcher() {
   await sofiaDispatcher(this.client, this.client.kazagumo);
  }
              ...
              .map(async (cmd) => {
                ...
                try {
                  ...
                  if (command && typeof command === "object" && command.name) {
                    ...
                    console.error(
                      `Failed Loading Command: Invalid structure for command in file: ${cmd}`
                    );
                  }
                } catch (importError) {
                  console.error(
  ...
  validateAndStoreCommand(command, client) {
    if (
      ...
      console.error(
        `Failed Loading Command: Invalid structure for command: ${command?.name || 'undefined'}`
      );
    }
  }
};
