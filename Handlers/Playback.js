...
module.exports = class Music {
  ...
  async handleError(ctx, err) {
    if (ctx.type === InteractionType.ApplicationCommand) {
      await ctx.editReply({
        content:
          "- An error occurred while playing the song. Try Reporting it to the Developer [Team Sofia](<https://discord.gg/hindustani>)"
      });
  ...
  async Play(ctx, query, source) {
    try {
      ...
      const player = await this.CreatesofiaPlayer(ctx);
  ...
  async Skip(ctx) {
    try {
      ...
      player.skip();
      const newMsg = `<a:yes:1265666029091029104> **${this.Name(
        ctx
  ...
  async Disconnect(ctx) {
    try {
      ...
      // }
      const newMsg = `<a:yes:1265666029091029104> **Disconnected** From Voice Channel!`;
      await this.ctxSend(ctx, { content: newMsg });
    } catch (err) {
      this.console.error(err);
    }
  ...
  async Queue(ctx, p) {
    try {
      ...
      if (player && player.queue.size == 0 && player.playing) {
        // const msg = `## <a:music1:1314190805153480774> Queue Of ${
        //   ctx.guild.name
  ...
  \`\`\``;
      ...
      const QueuePage = `## <a:music1:1314190805153480774> Queue Of ${
        ctx.guild.name
      }\n${
      ...
      collector.on("collect", async (i) => {
        ...
        if (i.customId === "previous_q") {
          ...
          const QueuePage = `## <a:music1:1314190805153480774> Queue Of ${ctx.guild.name}\n${pageMsg}\n\n**Page ${page}/${totalPages}**`;
