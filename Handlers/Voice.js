...
class VoiceStateHandler {
    ...
    async handleLeave(newState, is247Enabled) {
        ...
        if (dd.length === 0) {
            if (ryzenTimer.has(newState.guild.id)) {
                clearTimeout(ryzenTimer.get(newState.guild.id));
            }
            const timer = setTimeout(async () => {
                ...
                ryzenTimer.delete(newState.guild.id);
            }, 300000);
            ryzenTimer.set(newState.guild.id, timer);
        }
    }
    ...
    async destroyPlayer(guildId) {
        ...
        if (player) {
            try {
                const channel = await this.sofiaChannel(guildId);
                if (channel) {
                    const embed = new EmbedBuilder()
                        .setColor(this.client.config?.color || '#FF66CC')
                        .setAuthor({name:`sofia Music System`,iconURL:this.client.user.displayAvatarURL()})
