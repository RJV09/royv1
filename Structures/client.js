...
const SlashLoader = require("../Handlers/SlashLoader");
...
module.exports = class ryzenClient extends Client {
  constructor() {
    let shardingOptions = {};
    try {
  ...
  async ryzenBuild() {
    try {
      ...
      console.error("Error during sofiaBuild:", error);
    }
  }

  async loadSlashCommands() {
    const slashLoader = new SlashLoader(this);
    await slashLoader.loadSlashCommands();
    this.slashLoader = slashLoader;
  }
  ...
  async connectToDatabase() {
    ...
    // Check if we have valid MongoDB URLs
    if (!this.config.mongoURL.Primary || this.config.mongoURL.Primary.includes('localhost') || this.config.mongoURL.Primary.includes('127.0.0.1')) {
      console.log("No valid MongoDB URL found. Please set up MongoDB Atlas or another hosted MongoDB service.");
      console.log("Add your MongoDB connection string to the .env file as PRIMARY_MONGO_URL");
      return;
    }
    ...
    await mongoose
      ...
      .catch(async (error) => {
        ...
        if (this.config.mongoURL.Secondary && !this.config.mongoURL.Secondary.includes('localhost') && !this.config.mongoURL.Secondary.includes('127.0.0.1')) {
          await mongoose
            ...
            .catch((error) => {
              ...
              console.log("Please check your MongoDB connection strings in the .env file");
            });
        } else {
          console.log("Please set up a valid MongoDB connection string in your .env file");
        }
      });
  }
};